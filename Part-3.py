...
def three_layer_convnet(x, params):
    ...
    ############################################################################
    paddedX=tf.pad(x,[[0,0],[2,2],[2,2],[0,0]],'CONSTANT')
    layer1=tf.nn.relu(tf.nn.conv2d(paddedX,conv_w1,strides=[1, 1, 1, 1], padding='VALID')+conv_b1)
    
    paddedX2=tf.pad(layer1,[[0,0],[1,1],[1,1],[0,0]],'CONSTANT')
    layer2=tf.nn.relu(tf.nn.conv2d(paddedX2,conv_w2,strides=[1, 1, 1, 1], padding='VALID')+conv_b2)
    scores=tf.matmul(flatten(layer2),fc_w)+fc_b
    ############################################################################
    #                              END OF YOUR CODE                            #
...
class ThreeLayerConvNet(tf.keras.Model):
    def __init__(self, channel_1, channel_2, num_classes):
        ...
        initializer = tf.variance_scaling_initializer(scale=2.0)
        stride=[1,1]
        self.conv1 = tf.layers.Conv2D(channel_1,[5,5],stride,padding="VALID",use_bias=True, activation=tf.nn.relu,bias_initializer=tf.zeros_initializer(),kernel_initializer=initializer)
        self.conv2 = tf.layers.Conv2D(channel_2,[3,3],stride,padding="VALID",use_bias=True, activation=tf.nn.relu,bias_initializer=tf.zeros_initializer(),kernel_initializer=initializer)        
    ...
    def call(self, x, training=None):
        ...
        ########################################################################
        paddedX=tf.pad(x,[[0,0],[2,2],[2,2],[0,0]],'CONSTANT')
        x = self.conv1(paddedX)
        paddedX2=tf.pad(x,[[0,0],[1,1],[1,1],[0,0]],'CONSTANT')
        x = self.conv2(paddedX2)
        x = tf.layers.flatten(x)
        scores=self.fc(x)
        ########################################################################
        #                           END OF YOUR CODE                           #
...
def model_init_fn(inputs, is_training):
    ...
    layers = [
        ...
        tf.keras.layers.Conv2D(channel_1, [5,5], stride,use_bias=True,bias_initializer=tf.zeros_initializer(), padding='same',kernel_initializer=initializer,activation=tf.nn.relu),
        tf.keras.layers.Conv2D(channel_2, [3,3], stride,use_bias=True,bias_initializer=tf.zeros_initializer(), padding='same',kernel_initializer=initializer,activation=tf.nn.relu),
...
def model_init_fn(inputs, is_training):
    ...
    ############################################################################
    channel_1, channel_2, num_classes = 64, 64, 10
    initializer = tf.variance_scaling_initializer(scale=2.0)
    input_shape = (32,32,3)
    stride=[1,1]
    layers = [
        tf.keras.layers.InputLayer(input_shape=input_shape),
        tf.keras.layers.Conv2D(channel_1, [3,3], stride, padding='same',kernel_initializer=initializer,activation=tf.nn.relu),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(0.8),
        tf.keras.layers.MaxPool2D([2,2], [2,2]),
        
        tf.keras.layers.Conv2D(64, [3,3], stride, padding='valid',kernel_initializer=initializer,activation=tf.nn.relu),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(0.8),
        tf.keras.layers.MaxPool2D([2,2], [2,2]),
        
        tf.keras.layers.Conv2D(channel_2, [3,3], stride, padding='valid',kernel_initializer=initializer,activation=tf.nn.relu),
        tf.keras.layers.BatchNormalization(),
        tf.keras.layers.Dropout(0.8),
        tf.keras.layers.AveragePooling2D([2,2], [2,2]),
...
# 
# - Here we used the **Keras Sequential API** to model above network as a stack of layers.
# - In order to get the optimzed accuracy , we have used the **Adam Optimizer**.
# - We have used 3 convolution layers each having 64 filters followed by ReLU activation function.
# - Each of the filter is of dimension $3x3$.
# - We also performed the pooling i.e. **max-pooling** to subsamples/downgrade the feature maps of first two convolution layers and **average-pooling**  to subsample the feature map of third convolution layer.
# - Dimension of max-pool and avg-pool window is $2x2$.
# - We also performed the normalization using **Batch Normalization**.
# - We also performed the regularization to avoid the overfitting using **Dropout**.
# - Finally we applied the fully connected layer of $32$ neurons after flatten out the subsample and forward its result to the output layer of $10$ neurons.
# 
